...
src/
  components/
    ...
    layout/
      ...
      Sidebar.jsx         — fetches real chat list via `getChats()` on mount and re-fetches on `chatRefresh` prop change. Renders loading/empty/list states. Chat cards use `bg-[#2F3347]` with `overflow-y-auto` scrolling. "Account Settings" entry in settings mode. In friendsMode: fetches friends list + pending requests on mount and re-fetches both on `friendsRefresh` prop change, renders request blocks with accept/reject (Check/X icons), and friends list with click-to-chat via `createChat`. `handleFriendClick` calls `toggleFriendsMode()` after opening chat.
    chat/
    ...
    friends/
      FriendsView.jsx     — search input + debounce + send friend request on user card click. Filters self out of search results via `props.userName`. Shows "Request Sent" confirmation after sending. `handleUserClick` awaits `sendFriendRequest(user.id)` and only marks the card on success or a known duplicate (409), not on network failure.
    settings/
      SettingsPanel.jsx   — big settings icon by default; account options list (username/password/email/profile picture) when activeSettings prop is true. Fully wired: fetches current profile via `getMe` on mount, `openModal(option)` opens a fixed-overlay modal per option (prefills username/email from profile), submits via `updateUsername`/`updateEmail`/`updatePassword` with inline backend error display. Username change saves the re-issued JWT to localStorage and calls `setUserName` (threaded from App via HomePage) so WelcomeView/App state update immediately. Profile picture option not wired yet (needs upload UI).
    api/
      client.js           — axios calls to backend: loginDataPython(data), signUpDataPython(data), verifyToken(token), search(query), searchFriends(query), createChat(user2Id), sendMessages(chatId, content), getMessages(chatId), getFriendsList(), getFriendRequests(), sendFriendRequest(user_id), acceptFriendRequest(request_id), rejectFriendRequest(request_id), deleteFriend(friend_id), getMe(), updateUsername(username), updateEmail(email), updatePassword(current_password, new_password), updateProfilePic(image_url). Returns { auth, message, token } or network-error fallback. search returns response.data (list of user dicts) or null on network error. searchFriends returns { auth, friends } with JWT auth. sendFriendRequest returns `error.response.data` on HTTP errors (so a 409 duplicate reaches the caller) or `{ auth: false, message: "Network error. Try again." }`. createChat/sendMessages/getMessages send JWT in Authorization header. Exports `API_BASE` and `WS_BASE` constants at the top — change `localhost` in `VITE_PERSONAL_IP` to laptop IP in root `.env` for phone testing. (Moved from SendDataPython.jsx)
  pages/
    AuthPage.jsx          — toggles LoginForm/SignUpForm via showSignUp state + toggleSwitch (prevState => !prevState pattern)
      HomePage.jsx          — owns chatMode, welcome, activeSettings, selectedChat, chatRefresh, friendsRefresh, friendsMode state. Renders NavBar + Sidebar + (ChatView | WelcomeView | NewMessage | SettingsPanel | FriendsView) depending on state combo. `handleBack` sets `selectedChat(null)` to return to WelcomeView/NewMessage. `goHome` resets all state (closes chat, shows Welcome, switches to chat mode). `bumpChatRefresh()` increments `chatRefresh` (triggers Sidebar chat re-fetch) and `bumpFriendsRefresh()` increments `friendsRefresh` (triggers Sidebar friends/requests re-fetch). Passes `userName` to ChatView (isMine detection) and FriendsView (self-filtering). `toggleFriendsMode` toggles friendsMode boolean (same prev => !prev pattern). Receives `setUserName` from App and forwards it to SettingsPanel so username changes propagate to App state. On mount opens a global `/ws/notifications` WebSocket (built from `WS_BASE` + JWT token query param) via a `connect()` helper using refs (`wsRef`, `genRef`, `reconnectAttemptRef`, `reconnectTimerRef`, `isIntentionalCloseRef`). `onmessage` maps `new_message` → `bumpChatRefresh()` and `new_friend_request` → `bumpFriendsRefresh()`. Reconnects with exponential backoff (max 5 attempts, delay 1s × 2^n) unless the close was intentional; generation counter (`genRef`) guards against stale handlers. Cleans up socket + timers on unmount.
  App.jsx                 — React Router routes: "/" → AuthPage, "/home" → HomePage (guarded by isAuthenticated), "*" → redirect to "/". Owns isAuthenticated + userName + loading state, restores session via /verify on mount. Passes both `userName` and `setUserName` to HomePage — username changes re-issue a JWT and must update App state.
...
- `chatRefresh` counter in HomePage: bumping it (`setChatRefresh(prev => prev + 1)`) triggers Sidebar's chat `useEffect` to re-fetch chat list. `bumpChatRefresh` function is passed to `ChatView` (called after send) and `NewMessage` (called after chat creation) so the sidebar updates in real-time.
- `friendsRefresh` counter in HomePage works the same way (`setFriendsRefresh(prev => prev + 1)`), triggering Sidebar's friends + requests re-fetch (both effects keyed on `props.friendsRefresh`). Bumped by the notification socket when a `new_friend_request` arrives.
- Live push-notification pattern: a single global WebSocket (`/ws/notifications`) carries events when the app is NOT inside a chat. Unlike the per-chat socket, no content is exchanged — just `{ type }` signals that bump the appropriate refresh counter so Sidebar re-fetches. The socket lives in HomePage; ChatView's per-chat socket stays independent. Because `connect()` is recreated on every render, its `useEffect` deliberately has an `[]` deps array with a `// eslint-disable-next-line react-hooks/exhaustive-deps` suppression when needed — adding `connect` to deps would close/reopen the socket every render.
...
- `POST /friends/request/{user_id}` — JWT auth → send friend request (status: pending). Returns 200 `{ auth, message }` and pushes a `new_friend_request` notification to the recipient via `NotificationManager`. Prevents self-requests (`from_id == to_id` returns None) and duplicate requests in either direction (`(A→B)` or `(B→A)` via SELECT check before INSERT, returns `409 Conflict`).
...
- WebSocket `/ws/notifications` — connects via JWT token as query param, verifies the token and that the user exists (closes 1008 otherwise), then registers the socket in `NotificationManager` keyed by `user_id`. The server pushes `new_friend_request` (from `POST /friends/request/{user_id}`) and `new_message` (from the `/ws/{chat_id}` message/image branches via `get_other_participant`). **Must be declared BEFORE `/ws/{chat_id}` in `routers/ws.py`** — FastAPI matches routes in order and `/ws/{chat_id}` uses a `StringConvertor` (`[^/]+`) that greedily matches `/ws/notifications`, which returned 403 (the fix for route shadowing).
- WebSocket `/ws/{chat_id}` — connects via JWT token as query param, verifies the caller is a participant in the chat (closes with code 1008 otherwise), broadcasts new messages to all connected clients in the chat room in real-time via `ConnectionManager`. Handles `message`, `image`, `typing`, `stop_typing`, `call_offer`, `call_answer`, `ice_candidate`, `call_end`, and `call_reject` event types. `image` type accepts `image_url` (from prior upload) and optional `content` caption. On `message`/`image` also pushes a `new_message` notification to the other participant via `NotificationManager`. Typing, stop_typing, and all VoIP signaling events broadcast to others only (excludes sender via `exclude` parameter on `broadcast`). VoIP message types forward `data` payload (SDP offer/answer, ICE candidate) without inspecting it — the backend is purely a signaling relay.
...
- `database.py` functions: `create_connection`, `create_table`, `insert_user`, `get_user_hash`, `get_user_hash_by_id`, `get_user_profile`, `update_username`, `update_email`, `update_password`, `update_profile_picture`, `search_users`, `create_chat`, `get_chat_passkey_hash`, `get_user_by_username`, `get_user_by_id`, `insert_message`, `get_messages_by_chat_id`, `get_last_message_by_chat_id`, `get_chats_by_user_id`, `get_user_suggestions`, `send_friend_request`, `accept_friend_request`, `reject_friend_request`, `get_pending_requests`, `get_friends`, `remove_friend`, `search_friends`, `chat_belongs_to_user`, `get_other_participant`
- `db/schemas.py` — Pydantic request body models (`UserSignUp`, `UserLogin`, `TokenRequest`, `CreateChat`, `CreateMessage`, `UpdateUsername`, `UpdateEmail`, `UpdatePassword`, `UpdateProfilePicture`)
- `NotificationManager` in `core/connection_manager.py` — per-user (not per-chat) WebSocket registry for push notifications. Methods: `connect(websocket, user_id)`, `disconnect(websocket, user_id)`, `send_to_user(message, user_id)` — **argument order is `(message, user_id)`, message first**. Exposed globally as `state.notification_manager`, initialized in `main.py`.
...
- ~~`messages.timestamp` stored UTC while WebSocket broadcasts used local time~~ → fixed, messages table schema changed to `datetime('now', 'localtime')`. (`chats.created_at` / `friends.created_at` still UTC — not user-visible, so left as-is)
...
- ~~`/ws/notifications` always returned 403 even after restarts~~ → route shadowing: `/ws/{chat_id}` uses a `StringConvertor` regex `^/ws/(?P<chat_id>[^/]+)$` that matches ANY segment including `notifications`, and it was registered BEFORE the notifications route, so Starlette bound `chat_id="notifications"`, membership check failed → close 1008 → 403. Fixed by declaring `/ws/notifications` BEFORE `/ws/{chat_id}` in `routers/ws.py`.
- ~~`POST /friends/request/{user_id}` crashed with 500 "cannot use 'dict' as a dict key"~~ → `send_to_user` in `core/connection_manager.py` was defined as `(self, user_id, message)` but both call sites passed `(message, user_id)`. Fixed by changing the signature to `(self, message, user_id)` to match callers.
- ~~Frontend marked "Request Sent" even when the friend request failed~~ → `sendFriendRequest` in `client.js` now returns `error.response.data` on HTTP errors (was swallowing to `undefined`), and `FriendsView.handleUserClick` only adds a user id to `sentRequest` when the result is a success or a known duplicate (409), skipping actual network failures.
...
22. ~~Wire SettingsPanel to real settings endpoints~~ → done (username/email/password modals + `getMe` prefill, on `feature/settings-panel`). Username change saves the re-issued JWT to localStorage and updates App's `userName` state via the threaded `setUserName` prop. Profile picture still pending (needs image upload UI first).
...
24. ~~Live push notifications for new chats/messages + friend requests~~ → done. Backend: `/ws/notifications` endpoint + `NotificationManager` per-user registry, pushes `new_friend_request` (from `POST /friends/request`) and `new_message` (from chat WS message/image branches). Frontend: HomePage opens the socket on mount with exponential-backoff reconnect, `new_message` → `bumpChatRefresh()`, `new_friend_request` → `bumpFriendsRefresh()`.
